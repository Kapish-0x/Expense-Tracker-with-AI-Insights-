...
import React, { useState } from "react";
import { ShieldCheck, Target, Bell, Lock, Save, AlertTriangle, Trash2 } from "lucide-react";
import { useAuth } from "../store/authStore";
import axios from "axios";
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-8 animate-in fade-in duration-700">
      <header>
        ...
        <p className="text-slate-500">Configure financial guardrails and node security.</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
          ...
          <form onSubmit={handleUpdateBudget} className="space-y-5">
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Monthly Budget Limit</label>
              <input 
                type="number" 
                className="w-full bg-slate-50 border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                value={budgetData.monthlyBudget}
            ...
            <div className="space-y-1.5">
              ...
              <input 
                ...
                onChange={(e) => setBudgetData({...budgetData, minSavings: e.target.value})}
              />
              <p className="text-[10px] text-slate-400 italic mt-1">Dashboard alert triggers below this value.</p>
            </div>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-xl transition-all">
              <div>
                ...
                <p className="text-[10px] text-slate-400">Toggle dashboard alerts for low savings.</p>
              </div>
              <div 
              ...
              >
                <div className={`absolute top-1 w-3 h-3 bg-white rounded-full shadow-sm transition-all duration-300 ${budgetData.savingsAlertEnabled ? 'left-6' : 'left-1'}`}></div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-3 opacity-50 cursor-not-allowed">
              <div>
                ...
                <p className="text-[10px] text-slate-400">Receive EOW performance reports.</p>
              </div>
          ...
          <p className="text-[10px] text-center text-slate-400 mt-4 uppercase tracking-widest italic">Toggles sync instantly with your node.</p>
        </div>
        ...
        <div className="md:col-span-2 bg-slate-950 text-white p-8 rounded-[2.5rem] shadow-xl space-y-6">
          ...
          <form onSubmit={handleUpdateSecurity} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-[2px]">Current Access Key</label>
                <input 
                  type="password" 
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-emerald-500 outline-none transition-all placeholder:text-white/20"
                  placeholder="••••••••"
            ...
            <div className="bg-white/5 p-6 rounded-3xl border border-white/10 flex flex-col justify-center items-center text-center">
              <AlertTriangle className="text-amber-400 mb-2" size={32} />
              <p className="text-sm font-medium">Danger Zone</p>
              <p className="text-[11px] text-slate-400 mt-1 mb-4 max-w-[200px]">Terminating your node will permanently purge all financial records.</p>
              <button 
                type="button"
